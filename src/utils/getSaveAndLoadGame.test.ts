...
describe('test getSaveGame function', () => {
    ...
    it('should save different fragments correctly', () => {
        ...
        testSaveGame({
            ...
            guesses:[[], [{pl: 'test', en: 'test', role: 'red'}]]
        })
        ...
        testSaveGame({
            allClicked: {test: true}
        });

        expect(mockStorage.test.allClicked.test).toBeDefined();
    })
    ...
    it('should load game correctly', () => {
        ...
        expect(testSetAllClicked).toHaveBeenCalledWith(mockStorage.test.allClicked);
    });
